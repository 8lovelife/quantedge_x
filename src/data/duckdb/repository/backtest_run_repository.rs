...
impl BacktestRunHistoryRepository {
    ...
    pub fn create(&self, mut run: BacktestRunHistory) -> Result<BacktestRunHistory, duckdb::Error> {
        let conn = self.conn.lock().unwrap();

        let mut stmt = conn.prepare(
            r#"
            INSERT INTO strategy_backtest_run_history
            (strategy_id, parameters, market_details, status, start_time)
            VALUES (?, ?, ?, ?, ?)
            RETURNING id
            "#,
        )?;
    ...
    pub fn get_latest_top_run(
        ...
        strategy_id: i64,
        top: u32,
    ) -> Result<Vec<BacktestRunHistory>, duckdb::Error> {
        let conn = self.conn.lock().unwrap();
        let mut stmt = conn.prepare(
            r#"
            SELECT id, strategy_id, parameters, performance, status, start_time, end_time, created_at,market_details
            FROM strategy_backtest_run_history
            WHERE strategy_id = ?
        ...
        let runs = stmt
            .query_map(params![strategy_id, top], |row| {
                Ok(BacktestRunHistory {
                    id: row.get(0)?,
    ...
    pub fn get_by_id(&self, id: i64) -> Result<Option<BacktestRunHistory>, duckdb::Error> {
        let conn = self.conn.lock().unwrap();
        let mut stmt = conn.prepare(
            r#"
            SELECT id, strategy_id, parameters, performance, status, start_time, end_time, created_at,market_details
        ...
        let run: Option<BacktestRunHistory> = match stmt.query_row([id], |row| {
            Ok(BacktestRunHistory {
                id: row.get(0)?,
                strategy_id: row.get(1)?,
                parameters: row
    ...
    pub fn update(&self, run: &BacktestRunHistory) -> Result<(), duckdb::Error> {
        let conn = self.conn.lock().unwrap();
        conn.execute(
            r#"
            UPDATE strategy_backtest_run_history
            SET strategy_id = ?, parameters = ?, status = ?, start_time = ?, end_time = ?,market_details = ?,performance = ?
            WHERE id = ?
            "#,
            params![
                run.strategy_id,
    ...
    pub fn delete(&self, id: i64) -> Result<(), duckdb::Error> {
        ...
        conn.execute(
            r#"
            DELETE FROM strategy_backtest_run_history
            WHERE id = ?
            "#,
            [id],
        )?;
    ...
    pub fn get_all(&self) -> Result<Vec<BacktestRunHistory>, duckdb::Error> {
        let conn = self.conn.lock().unwrap();
        let mut stmt = conn.prepare(
            r#"
            SELECT id, strategy_id, parameters, status, start_time, end_time, created_at,market_details
            FROM strategy_backtest_run_history
            ORDER BY start_time DESC
            "#,
        )?;
